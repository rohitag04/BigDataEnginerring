...
## see ../data/README.txt
college <- read.csv("C:/Users/rohit/Desktop/College.csv")
...
## [your code here] #this will give you all grad rate in descending order
data <- select(df, X, Grad.Rate)
result <- arrange(data, desc(Grad.Rate))
View(result)
...
ans <- group_by(df, Private)
ans <- summarise(ans, max.fee= max(Outstate), min.fee = min(Outstate))
View(ans)
...
## [your code here]
ans1 <- group_by(college, Private)
ans1 <- summarise(ans1, max.fee= max(Outstate), min.fee = min(Outstate))
View(ans1)
